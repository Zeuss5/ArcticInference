...
from arctic_inference.common.swiftkv.configs import Qwen3SwiftKVConfig
...
def get_attn_metadata_for_swiftkv():
    ...
    assert all(m is meta for m in fwd_ctx.attn_metadata.values()), \
        "All attention metadata should be the same for Qwen3SwiftKV."
    return meta


class Qwen3SwiftKVAttention(Qwen3Attention):
    ...
    def __init__(
        ...
        # Add QK-Norm layers for SwiftKV projections
        self.q_norm_swiftkv = RMSNorm(self.head_dim, eps=rms_norm_eps)
        self.k_norm_swiftkv = RMSNorm(self.head_dim, eps=rms_norm_eps)
    ...
    def forward(
        ...
        q_by_head = self.q_norm_swiftkv(q_by_head)
        q = q_by_head.view(q.shape)
        
        # Apply rotary embedding (k already has k_norm applied in prefill)
...
class Qwen3SwiftKVPrefillRunner(nn.Module):

    def __init__(self, *, vllm_config: VllmConfig, model: "Qwen3SwiftKVModel",
                 prefix: str = ""):
        super().__init__()
    ...
    @property
    def model(self) -> "Qwen3SwiftKVModel":
    ...
    def forward(
        ...
        for layer in self.model.layers[self.config.num_key_value_layers:]:
            ...
            # Apply QK-Norm to k (critical for Qwen3!)
            k_by_head = k.view(*k.shape[:-1], k.shape[-1] // layer.self_attn.head_dim, layer.self_attn.head_dim)
            k_by_head = layer.self_attn.k_norm_swiftkv(k_by_head)
...
@support_torch_compile
class Qwen3SwiftKVDecodeRunner(nn.Module):
...
class Qwen3SwiftKVModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        with model_runner.set_shift_parallel_mode(True):
            self.layers.extend([
                Qwen3SwiftKVDecoderLayer(config=config,
                                         cache_config=vllm_config.cache_config,
                                         quant_config=vllm_config.quant_config,
                                         prefix=f"{prefix}.layers.{idx}")
                for idx in range(config.num_key_value_layers,
    ...
    def _init_prefill_runner(self, vllm_config: VllmConfig):
        ...
        self.prefill_runner = Qwen3SwiftKVPrefillRunner(
    ...
    def _init_decode_runner(self, vllm_config: VllmConfig):
        ...
        vllm_config.compilation_config.inductor_compile_config = (
            vllm_config.compilation_config.inductor_compile_config.copy())
        self.decode_runner = Qwen3SwiftKVDecodeRunner(
            vllm_config=vllm_config, model=self)
    ...
    def swiftkv_select(
        ...
        # DEBUG: Print token survival rate
        original_tokens = hidden_states.shape[0]
        survival_rate = num_surviving_tokens / original_tokens if original_tokens > 0 else 0
        print(f"[SwiftKV DEBUG] Original tokens: {original_tokens}, Surviving: {num_surviving_tokens}, Rate: {survival_rate:.2%}")
...
class Qwen3SwiftKVForCausalLM(nn.Module):
    ...
    def _init_model(self,
                    vllm_config: VllmConfig,
                    prefix: str = ""):
        return Qwen3SwiftKVModel(vllm_config=vllm_config, prefix=prefix)
